...
def cifar():
    ...
    models, names = pretrained.resnet18_collection_trained_on_cifar10(return_names=True, device='cuda:2', eval_=True)
...
def uci():
    ...
    sh.ensemble_sweep(test_samples=[10, 20, 50], test_seeds=range(1000), shifts=(True, False))
    sh.bbsd_sweep(test_samples=[10, 20, 50], test_seeds=range(1000), shifts=(True, False))
    sh.print_results()
